...
{"id": str, "sv": "LSST"|"ZTF", "b": "det"|"alert", "ins": bool (in metaDEBASS training/calibration),
 ...
 "mdb": {"sn": [P(supernova)], "ia": [P(SN Ia)]},   // metaDEBASS fusion v13b confidences; "ia" for ZTF only
 "q": {classifier: [metaDEBASS trust in that call or null]}}   // empty in this release (trust not yet calibrated for this catalogue)
```
...
| mdb_psn, mdb_pia, mdb_ndet, mdb_sv, mdb_ins | metaDEBASS fusion v13b calibrated P(supernova) and P(SN Ia) (ZTF only) at its latest scored detection, that detection number, the survey of the scored ID, and whether that ID was in metaDEBASS training/calibration; null if not scored. No class call: metaDEBASS is a meta-layer |
