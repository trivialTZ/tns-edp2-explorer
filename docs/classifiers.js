...
(function () {
  ...
  function render() {
    ...
    root.innerHTML = '<div class="wrap"><article class="prose wide sc-page">' +
      ...
      '<li><strong>metaDEBASS is not graded here.</strong> It is a meta-layer, not another classifier: for each object it reports calibrated confidences for follow-up ranking (P(supernova), and P(SN Ia) for ZTF) and how far to trust each broker’s call (trust is not shown yet: its levels are not calibrated for this catalogue). ' +
      'Object pages show the confidences. Its own benchmark, on objects it never trained on, is in the <a href="https://github.com/trivialTZ/rubin_hackathon" target="_blank" rel="noopener noreferrer">metaDEBASS repository</a>: on live Rubin alerts it separates supernovae from other transients well, and its Ia-versus-other separation is not yet better than chance.</li>' +
